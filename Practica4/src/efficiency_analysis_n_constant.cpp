...
int main() {
  ...
  std::ofstream outputFile("./data/efficiency_analysis_n_constant.txt");

  std::streambuf* coutBuffer = std::cout.rdbuf();
  std::cout.rdbuf(outputFile.rdbuf());

  for (int i = start_value; i < 10000; i += increment) {
    DealerBag dealerBag;
    dealerBag.initializeBenchmark(10000, i);
  ...
  std::ifstream inputFile("./data/efficiency_analysis_n_constant.txt");
  std::ofstream filteredOutputFile("./data/efficiency_analysis_n_constant_filtered.txt");
